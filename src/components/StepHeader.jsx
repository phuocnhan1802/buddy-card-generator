...
export default function StepHeader({ current }) {
  return (
    <header className="w-full border-b border-line bg-white/80 backdrop-blur px-6 lg:px-16 py-4 flex items-center justify-between">
      <div className="flex items-center gap-2">
        <div className="h-7 w-7 rounded-sm bg-coral" aria-hidden="true" />
        <span className="font-display font-bold text-ink text-sm tracking-tight">{config.WORKSHOP_NAME}</span>
      </div>

      <ol className="flex items-center gap-3 lg:gap-6">
        {STEPS.map((s) => {
